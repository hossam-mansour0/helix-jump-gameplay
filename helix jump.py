...
for _ in range(1, 25):
    r_angle_safe.append(randrange(0, 180, 35))
    r_angle_glue.append(randrange(0, 100, 20))
    angle_air.append((random.uniform(.5, .75)))
lose = False
flag = True
c = y
SCORE = 0
...
def draw_sphere(x1, y1, r_angle):
    ...
    glColor(0, 0, 1)
    glTranslate(.8 * sin(x1), y1 + .15, .8 * cos(x1))
...
def draw_circular_sectors():
    for i in range(0, 25):
        ...
        if i != 0:
            ...
            draw_Circular_sector(i, 2.9 - (.2 * LEVEL), r_angle_glue[i] + r_angle_safe[i])
    ...
    if y > 5:
        ...
        if y - 5 <= yy + .05:
            ...
            for ang in np.arange(3 - (.2 * LEVEL), 2 * pi - (3 - (.2 * LEVEL)), 0.005):
                x_check = round(sin(ang + (r_angle_safe[yy] + r_angle_glue[yy]) * pi / 180), 2) == round(sin(X), 2)
                z_check = round(cos(ang + (r_angle_safe[yy] + r_angle_glue[yy]) * pi / 180), 2) == round(cos(X), 2)
                if x_check and z_check:
                    lose = True
                    break
            # safe sector collision
            for ang in np.arange(angle_air[yy], 2 * pi - angle_air[yy], 0.005):
                x_check = round(sin(ang + r_angle_safe[yy] * pi / 180), 2) == round(sin(X), 2)
                z_check = round(cos(ang + r_angle_safe[yy] * pi / 180), 2) == round(cos(X), 2)
                if x_check and z_check:
                    flag = False
                    break
...
def ball_anim():
    ...
    elif flag is False and y < floor(y) + .67:  # ball bounce on red sectors
        # v_velocity = v_velocity + 9.8 * dtime
...
def add_tex(x1, x2, y1, y2, index1=0, z_near=-1, z_far=1):
    glPushMatrix()
    glMatrixMode(GL_MODELVIEW)
    glLoadIdentity()
    glMatrixMode(GL_PROJECTION)
...
def draw_sectors_with_light():
    for i_ in range(0, 25):

        # safe sectors
        white_material(.6, 0, 0, 0, .8, 0, 0)
        draw_Circular_sector(i_, angle_air[i_], r_angle_safe[i_])
        ...
        if i_ != 0:
            ...
            # to be visible
            glTranslate(0, 0.0001, 0)
            draw_Circular_sector(i_, 3 - (.2 * LEVEL), r_angle_glue[i_] + r_angle_safe[i_])
            glPopMatrix()
            after_white_material()
...
def draw():
    ...
    if START:
        ...
        add_tex(-1, 1, -1, 1, 0, z_near=-1000, z_far=10)
